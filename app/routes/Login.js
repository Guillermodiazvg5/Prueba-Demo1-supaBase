...
export const action = async ({ request }) => {
  ...
  const result = await clientSupaBase.from('saludos').insert([
    { saludo: saludo , autor: usuario }
  ]);
...
if (result.error) {
 ...
 const fieldErrors = {
  ...
  saludo: saludo.length < 9 ? 'El saludo debe contener mas de 9 caracteres ' : null,
  usuario: usuario.length < 4 ? 'El nombre de usuario debe contener mas de 4 caracteres ' : null
